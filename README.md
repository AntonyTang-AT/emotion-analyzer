...
> 基于六层流水线架构的 AI 情感分析系统，支持**视频 / 纯音频 / 纯文本 / 纯图片**等多种输入源，可进行多模态分歧分析、**动态路由权重**、生成式报告与大五人格回归（部分输入源下降级或跳过 L4/L6）。
...
- 🎛️ **动态路由**：L4 以连续分歧分数和置信度为主信号，保留矛盾类型解释并输出可审计的 `suggested_fusion_weights`。
...
| L4 | 分歧分析 + 动态路由 | VA 距离 + 规则解释 + 查表 fallback + 选择性门控，输出权重、决策与审计轨迹 |
| L5 | 报告生成 | 默认本地 Qwen，基于 L4 已锁定结果生成片段/整体报告 + 可视化 |
